...
def _simple_replace(content: str, old: str, new: str, replace_all: bool) -> tuple[str | None, int]:
    """Exact string match and replace.
    Returns (new_content, match_count) or (None, 0)."""
    count = content.count(old)
    if count == 0:
        return None, 0
    if replace_all:
        return content.replace(old, new), count
    return content.replace(old, new, 1), count


def _line_trimmed_replace(content: str, old: str, new: str, replace_all: bool) -> tuple[str | None, int]:
    """Strip trailing whitespace from each line before matching.
    Builds normalised version and replaces in original content."""
    ...
    if replace_all:
        result = norm_content.replace(norm_old, norm_new)
    else:
        result = norm_content.replace(norm_old, norm_new, 1)

    # Restore original line endings (the normalised version uses \n)
    # Since we're on Linux and generate UTF-8 files this is fine.
    return result, count


def _whitespace_normalized_replace(content: str, old: str, new: str, replace_all: bool) -> tuple[str | None, int]:
    """Collapse all whitespace runs to a single space for matching.
    Handles indentation drift and mixed spaces/tabs."""
...
# Ordered chain: try each strategy in turn
_STRATEGIES = [
...
def edit_file(path: str, edits: list[dict], workspace: Path) -> dict:
    ...
    Returns status dict with path and count of applied edits."""
    ...
    for i, edit in enumerate(edits):
        ...
        for strategy_name, strategy_fn in _STRATEGIES:
            ...
            if not replace_all and count > 1:
                return {
                    "ok": False,
                    "error": (
                        f"Edit #{i + 1}: Found {count} matches using {strategy_name}. "
                        "Provide more context in 'old_text' or set replace_all=true."
                    ),
                }
        ...
        if not matched:
            return {
                "ok": False,
                "error": (
                    f"Edit #{i + 1}: 'old_text' not found in {path} "
                    "(tried SimpleReplacer, LineTrimmedReplacer, WhitespaceNormalizedReplacer)"
                ),
            }
