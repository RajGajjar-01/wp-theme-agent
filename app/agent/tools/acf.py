...
def _build_location(template: str, scope: str) -> list:
    ...
    # front-page.php is auto-loaded by WordPress for the front page,
    # so use page_type instead of page_template (which requires manual selection)
...
def _build_field_group(
    ...
    def process_fields(field_list, parent_key):
        acf_list = []
        for field in field_list:
            fname = field.get("name", "")
            ftype = field.get("type", "text")
            flabel = field.get("label", fname.replace("_", " ").title())

            config = _get_field_config(ftype, fname, flabel)
            config["key"] = _generate_field_key(fname, parent_key)
...
def _update_functionsPhp(theme_dir: Path, scope: str, theme_slug: str) -> None:
    ...
    # ACF auto-loads JSON from the load_json path — no manual loading needed
    acf_code = """
...
def get_field_code(field_name: str, field_type: str, default: str = "") -> str:
    ...
    if field_type == "image":
        return f"""<?php $img = get_field('{field_name}'); ?><?php if($img): ?><img src="<?php echo esc_url($img); ?>" alt="<?php echo esc_attr(get_bloginfo('name')); ?>"><?php endif; ?>"""
    if field_type == "image_array":
        return f"""<?php $img = get_field('{field_name}'); ?><?php if($img): ?><img src="<?php echo esc_url($img['url']); ?>" alt="<?php echo esc_attr($img['alt']); ?>"><?php endif; ?>"""
    if field_type == "file":
        return f"""<?php $file = get_field('{field_name}'); ?><?php if($file): ?><a href="<?php echo esc_url($file['url']); ?>" download>Download</a><?php endif; ?>"""
    if field_type == "gallery":
        return f"""<?php $gallery = get_field('{field_name}'); ?><?php if($gallery): foreach($gallery as $img): ?><img src="<?php echo esc_url($img['sizes']['medium']); ?>" alt="<?php echo esc_attr($img['alt']); ?>"><?php endforeach; endif; ?>"""
    if field_type == "link":
        return f"""<?php $link = get_field('{field_name}'); ?><?php if($link): ?><a href="<?php echo esc_url($link['url']); ?>" class="btn"<?php if($link['target']): ?> target="<?php echo esc_attr($link['target']); ?>"<?php endif; ?>><?php echo esc_html($link['title']); ?></a><?php endif; ?>"""
    ...
    if field_type in ("wysiwyg", "oembed", "date_picker"):
        return f"""<?php echo get_field('{field_name}'); ?>"""
    if field_type in ("select", "radio", "button_group", "email"):
        return f"""<?php echo esc_html(get_field('{field_name}')); ?>"""
    if field_type == "checkbox":
        return f"""<?php $values = get_field('{field_name}'); ?><?php if($values): ?><?php foreach($values as $val): ?><span><?php echo esc_html($val); ?></span><?php endforeach; endif; ?>"""
    if field_type in ("number", "range"):
        return f"""<?php echo esc_html(get_field('{field_name}') ?: 0); ?>"""
    if field_type == "url":
        return f"""<?php $url = get_field('{field_name}'); ?><?php if($url): ?><a href="<?php echo esc_url($url); ?>">Link</a><?php endif; ?>"""
    if default:
